function Client() {
  ...
  this.history = new (globalThis as any).History(this)
  ...
  this.install = (host) => {
    ...
    this.theme.onLoad = () => { if (this.tile.hs) this.update() }
    ...
    this.acels.set('View', 'Toggle Menubar', 'CmdOrCtrl+Shift+E', () => { (window as any).electronAPI?.toggleMenubar?.() })
    ...
    ;(this.acels as any).pipe(this.commander)
  }

  this.loadExtensions = (extPath: string) => {
    try {
      const fs = require('fs')
      const path = require('path')
      if (!fs.existsSync(extPath)) return
      const OrcaTs = (globalThis as any).OrcaTs
      if (!OrcaTs) return
      const files = fs.readdirSync(extPath).filter((f: string) => f.endsWith('.js')).sort()
      for (const f of files) {
        try {
          const code = fs.readFileSync(path.join(extPath, f), 'utf8')
          ;(function (lib: typeof library, Op: typeof Operator, OT: typeof OrcaTs) {
            eval(code)
          })(library, (globalThis as any).Operator, OrcaTs)
  ...
  this.isNear = (x: number, y: number) => {
    return x > (Math.floor(this.cursor.x / this.grid.w) * this.grid.w) - 1 && x <= ((1 + Math.floor(this.cursor.x / this.grid.w)) * this.grid.w) && y > (Math.floor(this.cursor.y / this.grid.h) * this.grid.h) - 1 && y <= ((1 + Math.floor(this.cursor.y / this.grid.h)) * this.grid.h)
  }
  ...
  this.drawGuide = () => {
    ...
    const operators = Object.keys(this.library).filter((val) => isNaN(Number(val)))
    for (const id in operators) {
      const key = operators[id]
      const oper = new ((this.library as any)[key])(null, 0, 0, false)
  ...
  this.docs = () => {
    let html = ''
    const operators = Object.keys(library).filter((val) => isNaN(Number(val)))
    for (const id in operators) {
      const oper = new ((this.library as any)[operators[id]])(null, 0, 0, false)
