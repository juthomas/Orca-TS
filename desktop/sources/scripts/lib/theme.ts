function Theme(this: ITheme, _client: IClient) {
  this.el = document.createElement('style');
  this.el.type = 'text/css';

  this.active = {} as Record<string, string>;
  this.default = {
    background: '#eeeeee',
    f_high: '#0a0a0a',
    f_med: '#4a4a4a',
  ...
  this.install = (host: HTMLElement = document.body) => {
  ...
  this.start = () => {
    ...
    if (typeof localStorage !== 'undefined' && isJson(localStorage.theme)) {
      const storage = JSON.parse(localStorage.theme) as Record<string, string>;
      if (isValid(storage)) {
  ...
  this.set = (key: string, val: string) => {
    if (!val) return;
  ...
  this.parse = (any: unknown): Record<string, string> | undefined => {
    if (isValid(any as Record<string, string>)) return any as Record<string, string>;
    if (typeof any === 'string' && isJson(any)) return JSON.parse(any);
    if (typeof any === 'string' && isHtml(any)) return extract(any);
    return undefined;
  };

  this.drag = (e: DragEvent) => {
    e.stopPropagation();
    e.preventDefault();
  ...
  this.readFile = (file: File, callback: (data: string) => void): void => {
  ...
  function extract(xml: string): Record<string, string> | undefined {
  ...
  function isValid(json: Record<string, string> | null | undefined): boolean {
