function Commander(_client) {
    ...
    this.actives = {
        ...
        time: (p, origin) => {
            const formatted = new Date(250 * (client.orca.f * (60 / client.clock.speed.value)))
                .toISOString()
                .substr(14, 5)
                .replace(/:/g, '');
        ...
        color: (p) => {
            if (p.parts[0])
                client.theme.set('b_low', p.parts[0]);
            if (p.parts[1])
                client.theme.set('b_med', p.parts[1]);
            if (p.parts[2])
    ...
    this.trigger = function (msg = this.query, origin = null, stopping = true) {
        ...
        fn(new Param(val), origin ?? undefined);
    ...
    this.preview = function (msg = this.query) {
        ...
        if (!passives[cmd])
            return;
        passives[cmd](new Param(val));
    };
    this.onKeyDown = (e) => {
        if (e.ctrlKey || e.metaKey)
            return;
        client[this.isActive ? 'commander' : 'cursor'].write(e.key);
        e.stopPropagation();
    };
    this.onKeyUp = () => {
        client.update();
