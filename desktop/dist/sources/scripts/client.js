function Client() {
    ...
    this.history = new globalThis.History(this);
    ...
    this.install = (host) => {
        ...
        this.acels.set('Edit', 'Erase Selection', 'Backspace', () => { if (this.cursor.ins) {
            this.cursor.erase();
            this.cursor.move(-1, 0);
        }
        else {
            this[this.commander.isActive ? 'commander' : 'cursor'].erase();
        } });
        ...
        this.acels.set('Clock', 'Play/Pause', 'Space', () => { if (this.cursor.ins) {
            this.cursor.move(1, 0);
        }
        else {
    ...
    this.makeTheme = (type) => {
        ...
        // Output Reader
        if (type === 9) {
            return { bg: this.theme.active.b_inv, fg: this.theme.active.background };
        }
        // Reader+Background
        if (type === 10) {
    ...
    this.drawGuide = () => {
        ...
        for (const id in operators) {
            ...
            const oper = new (this.library[key])(null, 0, 0, false);
    ...
    this.resize = () => {
        ...
        if (tiles.w < bounds.w + 1) {
            tiles.w = bounds.w + 1;
        }
        if (tiles.h < bounds.h + 1) {
            tiles.h = bounds.h + 1;
        }
        this.crop(tiles.w, tiles.h);
        // Keep cursor in bounds
        if (this.cursor.x >= tiles.w) {
            this.cursor.moveTo(tiles.w - 1, this.cursor.y);
        }
        if (this.cursor.y >= tiles.h) {
            this.cursor.moveTo(this.cursor.x, tiles.h - 1);
        }
        const w = this.tile.ws * this.orca.w;
        const h = (this.tile.hs + (this.tile.hs / 5)) * this.orca.h;
        if (w === this.el.width && h === this.el.height) {
            return;
        }
    ...
    this.docs = () => {
        ...
        for (const id in operators) {
            const oper = new (this.library[operators[id]])(null, 0, 0, false);
