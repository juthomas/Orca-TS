function Clock(_client) {
    ...
    this.setSpeed = (value, target = null, setTimer = false) => {
        if (this.speed.value === value && this.speed.target === target && this.timer)
            return;
        if (value)
    ...
    this.stop = function (msg = false) {
        ...
        if (this.isPuppet === true) {
            ...
            if (msg === true || client.io.midi.isClock)
                client.io.midi.sendClockStop();
            this.clearTimer();
        }
    ...
    this.tap = function () {
        ...
        if (!this.isPuppet) {
            ...
            pulse.timer = setInterval(() => {
                if (performance.now() - (pulse.last ?? 0) < 2000)
                    return;
                this.untap();
            }, 2000);
        }
